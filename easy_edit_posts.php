...
function easy_finance_analysis($post)
{
    ...
    $ex        = array(
        'participant' => array(
            'Μέτοχος',
            'easy_finance_participants'
        ),
        'type' => array(
            'Τύπος',
            'easy_finance_doc_type'
        ),
        'category' => array(
            'Κατηγορία',
            'easy_finance_type'
    ...
    $sum       = $last_sum = array();
    $parts     = get_terms(array(
        'taxonomy' => 'easy_finance_participants',
        'hide_empty' => false
    ));
    $extra     = $res = array();
    if (function_exists('easy_finance_custom_rules_save')) {
        $res = easy_finance_custom_rules_save();
    }
        ...
        switch ($a) {
                    ...
                    switch ($aa) {
                        ...
                        case 'source':
                            ...
                            foreach ($b as $bb => $bbb) {
                                ...
                                foreach ($ark as $arko) {
                                    foreach ($ark2 as $pk) {
                                        if (!isset($sum['final'][$pk][$aa][$bb][$arko])) {
                                            $sum['final'][$pk][$aa][$bb][$arko] = 0;
                                        }
                                    }
                                }


                                $msg .= '<tr><td><strong>' . $tax_name[$bb] . '</strong></td><td>' . $bbb['amount'] . '<br /><small style="opacity:0.8;font-style:italic;">' . $sum['final']['income'][$aa][$bb]['amount'] . ' - ' . $sum['final']['expenses'][$aa][$bb]['amount'] . '</small></td><td>' . $bbb['vat'] . '<br /><small style="opacity:0.8;font-style:italic;">' . $sum['final']['income'][$aa][$bb]['vat'] . ' - ' . $sum['final']['expenses'][$aa][$bb]['vat'] . '</small></td><td>' . $bbb['final'] . '<br /><small style="opacity:0.8;font-style:italic;">' . $sum['final']['income'][$aa][$bb]['final'] . ' - ' . $sum['final']['expenses'][$aa][$bb]['final'] . '</small></td></tr>';
                                ...
                                if ($dd != 0) {
                                    foreach ($parts as $p) {
                                        ...
                                        if (isset($sum['final']['income'][$aa][$bb])) {
                                             if (!empty($extra) && !empty($extra['income'][$bb]) && function_exists('easy_finance_custom_rules_show')) {
                                                $results=easy_finance_custom_rules_show($extra['income'][$bb],$p->term_id);
                                                $namp=$results['namp'];
                                                $nvtp =$results['nvtp'];
                                                $nflp=$results['nflp'];
                                            } else {
                                        ...
                                        if (isset($sum['final']['expenses'][$aa][$bb])) {
                                            if (!empty($extra) && !empty($extra['expenses']) && function_exists('easy_finance_custom_rules_show')) {
                                        ...
                                        if (!isset($sum['final']['income']['participant'][$p->term_id][$bb])) {
                                            $sum['final']['income']['participant'][$p->term_id][$bb] = array(
                                                'vat' => 0,
                                                'amount' => 0,
                                                'final' => 0
                                            );
                                        ...
                                        if (!isset($sum['final']['expenses']['participant'][$p->term_id][$bb])) {
                                            $sum['final']['expenses']['participant'][$p->term_id][$bb] = array(
                                                'vat' => 0,
                                                'amount' => 0,
                                                'final' => 0
                                            );
            ...
            default:
                ...
                if (!empty($data)) {
                    ...
                    foreach ($data as $d) {
                        $count[$a][$akl][$d[$akl]][] = $count[$a]['source'][$d['source']][] = $count[$a]['type'][$d['type']][] = $count[$a]['participant'][$d['participant']][$d['type']][] =$count[$a]['client'][$d['client']][]= array(
                            'amount' => $d['amount'],
                            'vat' => $d['vat'],
                            'final' => $d['final']
                        );
                        if (empty($sum['final'])) {
                            $sum['final']['final']    = array(
                                'vat' => 0,
                                'amount' => 0,
                                'final' => 0
                            );
                            $sum['final']['income']   = array(
                                'vat' => 0,
                                'amount' => 0,
                                'final' => 0
                            );
                            $sum['final']['expenses'] = array(
                                'vat' => 0,
                                'amount' => 0,
                                'final' => 0
                            );
                    ...
                    foreach ($count[$a] as $dd => $vv) {
                        ...
                        switch ($dd) {
                            case 'participant':
                                ...
                                foreach ($vv as $kl => $lk) {
                                    ...
                                    foreach ($lk as $ll => $vs) {
                                        ...
                                        if (!isset($sum[$a]['participant'][$kl][$ll])) {
                                            $sum[$a]['participant'][$kl][$ll]          = array(
                                                'vat' => 0,
                                                'amount' => 0,
                                                'final' => 0
                                            );
                                            $sum['final'][$a]['participant'][$kl][$ll] = array(
                                                'vat' => 0,
                                                'amount' => 0,
                                                'final' => 0
                                            );
                                ...
                                break;
                            default:
                                $msg2 .= $dd!='client' ? '<tr><th>Τίτλος</th><th>' . $ex['amount'][0] . '</th><th>' . $ex['vat'][0] . '</th><th>' . $ex['final'][0] . '</th></tr>' : '';
                                foreach ($vv as $kl => $lk) {
                                    ...
                                    $msg2 .=$dd!='client' ?  '<tr><td>' . $tax_name[$kl] . '</td><td>' . $iamount . '</td><td>' . $ivat . '</td><td>' . $ifinal . '</td></tr>' : '';
                                    if ($dd == 'type' || $dd == 'source') {
                                        ...
                                        if (!isset($sum['final'][$a][$dd][$kl])) {
                                            $sum['final'][$a][$dd][$kl] = array(
                                                'vat' => 0,
                                                'amount' => 0,
                                                'final' => 0
                                            );
                        ...
                        if (!empty($res)) {
                            if (array_key_exists($dd, $res)) {
                                if ($a == $res[$dd]) {
                                    if (!isset($extra[$a][$d['type']][$kl])) {
                                        $extra[$a][$d['type']][$kl] = array(
                                            'amount' => 0,
                                            'final' => 0,
                                            'vat' => 0
                                        );
                                    }
                                    $extra[$a][$d['type']][$kl]['vat'] += $ivat;
                                    $extra[$a][$d['type']][$kl]['amount'] += $iamount;
                                    $extra[$a][$d['type']][$kl]['final'] += $ifinal;
                                }
                            }
